//Cylinder analysis
//Always subtract the lowest water molecule
...
double roundUp(double numToRound, double multiple);
...
int main(int argc,char* argv[])
{
    ...
    */
    ...
    //double monoLimits[2]={28.0,29.0}; For Sub951By50/Cyl30A without subtracting COM of substrate
    double monoLimits[2]={0.0,1.0};
    ...
    */
    ...
    int simPos=strrchr(argv[2],'C')+3-argv[2];
    int simLen=strrchr(argv[2],'A')-argv[2];
    double rDensCyl=atoi(string(argv[2]).substr(simPos,simLen).data())/2;
    ...
    //Sim box is either 50A or 100A long in y direction
    double dy=0;
    if(strstr(argv[2],"By50")>argv[2])
        dy=50;
    else
        dy=100;
    cout << "dy=" << dy << endl;
    ...
    double zlo=0;
    double zhi=0;
    //if(strstr(argv[2],"FullCyl")>argv[2])
    zhi=roundUp(10+8*rDensCyl,dz)+10;
    //else
    //    zhi=roundUp(10+4*rDensCyl,dz)+10;
    cout << "zhi = " << zhi << endl;
    int nz=(int) round((zhi-zlo)/dz);

    double dx=dz;
    double xlo=0;
    double xhi=roundUp(1.5*zhi,dx);
    int nx=(int) round((xhi-xlo)/dx);
    cout << "xhi = " << xhi << endl;

    //Bin volume
    double dV=2*dx*dy*dz;
    cout << "dV = " << dV << endl;
    ...
    /*
    double xlo=sqrt(alo/PI);
    double xhi=sqrt(ahi/PI);
    int nx=(int) round((xhi-xlo)/dr);
    ...
    */
    ...
    int cH=1200+28;
    int cW=(int) roundUp((int)floor(cH*(xhi-xlo)/(zhi-zlo)),16)+4;
    cout << "cW=" << cW%16 << endl;
    ...
    for(int i=0;i<=nA;i++)
    {
        aVals[i]=i*dA;
        vVals[i]=i*dV;
        rVals[i]=i*dr;//sqrt(aVals[i]/PI);
        pVals[i]=pow(((3*vVals[i])/(4*PI)),1.0/3);
    }
    */

    //Create Histograms
    TH2D *hA = new TH2D("hA","hA",nx,xlo,xhi,nz,zlo,zhi);
    TH1D *hMono = new TH1D("hMono","hMono",nx,xlo,xhi); //Only monolayer atoms - for calculating monoEdge
    //TH1D *hMonoProj;
    ...
    //Quiver
    Quiver *q = new Quiver(nx/2,xlo,xhi,nz/2,zlo,zhi);
    ...
    TLine *heightLine = new TLine(0,zlo,xhi,zhi); //Droplet height
    TLine *monoHiLine = new TLine(0,zlo,xhi,zhi); //top of monolayer
    TLine *monoLoLine= new TLine(0,zlo,xhi,zhi); //bottom of monolayer
    ...
    //Lines (There are 7)
    TLine *ldTanhLine = new TLine(xlo,zlo,xhi,zlo); //Liquid density (horizontal)
    ...
    //Z coordinate of lowest water molecule
    double lowestZ=100;
    ...
    while( (stepNum<=numSteps) && (loopFlag1) && (!inFile.eof()))
    {
        ...
        //Reset z coordinate of lowest water molecule
        lowestZ=100;
        ...
        //Get center of mass
        centerFile >> x0 >> y0 >> z0;
        //For cylindrical droplet, it seems like the monolayer doesn't fit nicely in .5A bins above the center of mass - shift by .25A upwards
        //Note that now the density numbers for the substrate in the densityHalfA should actually all be shifted up by 0.25A, so they're currently incorrect, though it won't actually matter
        z0-=0.25;
        //Go to next line
        centerFile.ignore(256,'\n');
        ...
        */
        ...
        for(int i=0;i<numAtoms;i++)
        {
            ...
            if(r[i]<=rDensCyl)
            {
                hWaterDens->Fill(z[i],convFact/(dZDens*rDensCyl*dy));
                nWaterDens++;
            }
            ...
            {
                hMono->Fill(r[i],convFact/(dx*dy*monoWidth));
                //nPotentialMonoAtoms++;
...
void findBoundaryPoints(TH2D* hist,TGraph *circlePointsGraph,char* aOrR,double *monoLimits,TH1D *hMono,double& rBulkMax,double &monoEdge,double &bulkEdge,int frameStep,TLegend* tanhLegend,TLine** tanhLines,TText **tanhTexts,TPaveText *tanhTextBox)
{
    ...
    TF1* tanhFit = new TF1("tanhFit","[0]/2*(1-tanh(4*(x-[2])/([1])))",0,500);

    //Set Bounds on parameters
    double fitBounds[6]={0.2,2.0,2,20,0,500};
    ...
    //Skip very first bin after monolayer
    firstBulkBin+=1;
    ...
    for(int i=1;i<=nx;i++)
    {
        ...
        tmpy=solveTanhFit(py,tanhFit,fitBounds,firstBulkBin,frameStep,bulkEdge,"col",center,tanhLegend,tanhLines,tanhTexts,tanhTextBox);
